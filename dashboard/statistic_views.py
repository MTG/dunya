...
def _common_stats(collectionid):
    ...
    if releases.count():
        ...
        num_recordings = models.CollectionFile.objects.filter(
            directory__musicbrainzrelease__collection__collectionid=collectionid
        ).count()
...
def carnatic_recordings(request):
    ...
    for c in concerts:
        ...
        tperf = carnatic.models.InstrumentPerformance.objects.filter(recording__concert=c, artist__in=artists)
        if not tperf.exists():
...
def carnatic_works(request):
    ...
    ret = {"nocomposer": nocomposer, "duplicatecomposer": duplicatecomposer, "all": works.order_by("title").all()}
...
def hindustani_recordings(request):
    ...
    for r in releases:
        ...
        tperf = hindustani.models.InstrumentPerformance.objects.filter(recording__release=r, artist__in=artists)
