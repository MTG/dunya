...
def tempogram_viaDFT(fn, param):
    ...
    TG, BPM, T = compute_fourierCoefficients(
        novelty, window, winLen - param.stepsize, param.BPM / 60.0, param.featureRate
    )
    BPM = BPM * 60.0
    T = T - T[0]
...
def getTempoCurve(tg, tcparams):
    ...
    for i in range(1, N):
        for jj in range(D):
            ...
            fnNow = (
                (DP[:, i - 1]).reshape((DP[:, i - 1]).size, 1) - tcparams.theta * bpmnow - tcparams.delta * penalArray
            )
...
if __name__ == "__main__":
    ...
    akCandLocs, akCandTs, akCandWts, akCandTransMat = estimateAksharaCandidates(
