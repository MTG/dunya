...
class RecordingList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            models.Recording.objects.with_permissions(collection_ids, permission)
            .select_related("work")
            .prefetch_related(
                "recordinginstrumentalist_set__artist",
                "recordinginstrumentalist_set__instrument",
                "performers",
                "shengqiangbanshi",
            )
        )
...
class ArtistList(generics.ListAPIView):
    ...
    def get_queryset(self):
        collection_ids = get_collection_ids_from_request_or_error(self.request)
        permission = utils.get_user_permissions(self.request.user)
        return models.Artist.objects.with_permissions(collection_ids, permission).select_related(
