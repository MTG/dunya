...
api_patterns = [
    ...
    re_path(
        rf"by-id/{external_identifier}/(?:add|update)/(?P<file_type>[a-z0-9-]+)",
        views.SourceFile.as_view(),
        name="ds-add-sourcetype",
    ),
    path("by-id/<uuid:uuid>/<slug:ftype>", views.download_external, name="ds-download-external"),
    re_path(rf"by-id/{uuid_match}\.(?P<ftype>mp3)", views.download_external, name="ds-download-mp3"),
...
urlpatterns = [
    ...
    path(
        "manager/delete-derived-files/<slug:slug>/<int:moduleversion>",
        views.delete_derived_files,
        name="docserver-delete-derived-files",
    ),
