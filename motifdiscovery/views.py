...
def artists(request):
    ...
    arts = carnatic.models.Artist.objects.filter(primary_concerts__recordings__mbid__in=mbids).distinct().order_by(
        'name')
    ret = {"artists": arts}
...
def seeds(request, uuid):
    ...
    matches = models.Match.objects.using('motif').filter(source__file__mbid=uuid).filter(version=-1).order_by(
        'distance').prefetch_related('source__segment').prefetch_related('source__file').prefetch_related(
        'target__file').prefetch_related('target__segment')
...
def results(request, uuid, seedid):
    rec = get_object_or_404(carnatic.models.Recording, mbid=uuid)
    matches = models.Match.objects.using('motif').filter(source=seedid, version=1).order_by(
        'distance').prefetch_related('source').prefetch_related('target').prefetch_related(
        'target__file').prefetch_related('target__segment')
    ret = {"recording": rec, "matches": matches}
